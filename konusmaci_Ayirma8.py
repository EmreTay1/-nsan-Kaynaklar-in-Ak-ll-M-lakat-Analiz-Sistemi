#TurEV-DB bu veri seti ile yapıcaz eğtimi tekrardan Türkçe veri seti ile eğitildi
#4 adet duyguyla türkçe denemeler yapıyorum.
...
#TurEV-DB bu veri seti ile yapıcaz eğtimi tekrardan.4 adet türkçe duygulu.
Funi tuning sonucları:
Step	Training_Loss	Validation_Loss	  Accuracy	     F1
500	   1.377500	          1.551312	   0.486957	      0.483890
1000	1.282800	     1.127840	     0.582609	   0.580458
1500	0.940100	     0.975277	     0.637681	      0.632659
📊 Doğruluk: 0.4870 | F1 Skoru: 0.4839
📊 Doğruluk: 0.5826 | F1 Skoru: 0.5805
📊 Doğruluk: 0.6377 | F1 Skoru: 0.6327
"""
...
#TurEV-DB veri setiyle yeniden eğitim yapıldı, 4 Türkçe duygu sınıfı kullanıldı.
Bu modelde istediğimiz çıktıyı alamadık malesef.
"""
...
LOCAL_DUYGU_MODEL = "./emre2_superverb_egitimli"  # Buraya senin local model klasörünü yaz
...
def duraksama_tespit(wav_yolu):
    ...
    print("Duraksama sayısı bulundu:", len(duraksamalar))
    return duraksamalar
...
def dolgu_tespit(wav_yolu):
    ...
    print("Dolgu sayısı bulundu (optimizasyon sonrası):", len(dolgu))
    return dolgu
...
def transkripsiyon_yap(wav_yolu, diarization, dil):
    ...
    for tur, _, konusmaci in diarization.itertracks(yield_label=True):
        ...
        print("İşlenen segment:", konusmaci, f"[{bas:.2f}-{bit:.2f}]")
        ...
        # 🔹 Local model duygu tahmini (etiket doğrudan modelden alınır)
        if extractor is not None and model_emotion is not None:
            try:
                ...
                with torch.no_grad():
                    ...
                    ses_tonu = model_emotion.config.id2label.get(tahmin, f"LABEL_{tahmin}")
            except Exception as e:
                ses_tonu = f"[Duygu tespiti hatası: {e}]"
    ...
    print("Transkripsiyon + ses tonu analizi tamamlandı.")
    return transkript
